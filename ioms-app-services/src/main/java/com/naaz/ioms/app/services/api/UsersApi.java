...
public class UsersApi {
    ...
    @Path(Constants.API_V1_VERSION + "/users/{id}/delete")
    ...
    @Path(Constants.API_V1_VERSION + "/users/{id}/update")
    public Response updateUser(@PathParam("id") final long id, @Valid @NotNull final Users user)
            throws IomsDbAccessException {
        try {
            usersDao.update(user);
            return Response.status(HttpStatus.SC_NO_CONTENT).build();
        } catch (Exception e) {
            log.error("failed to update user for id: {} due to exception.", id, e);
            throw new IomsDbAccessException(HttpStatus.SC_INTERNAL_SERVER_ERROR, "There was an error processing your " +
                    "request. It has been logged.");
        }
    }
